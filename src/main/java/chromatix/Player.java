...
import chromatix.AdventureSettings.Type;
import chromatix.api.UnintendedClientBehaviour;
import chromatix.api.UsedByReflection;
import chromatix.block.Block;
import chromatix.block.BlockBed;
import chromatix.block.BlockEndPortal;
import chromatix.block.BlockID;
import chromatix.block.BlockLiquid;
import chromatix.block.BlockRespawnAnchor;
import chromatix.block.BlockWood;
import chromatix.block.BlockWool;
...
import chromatix.blockentity.BlockEntitySign;
import chromatix.blockentity.BlockEntitySpawnable;
import chromatix.camera.PlayerCamera;
import chromatix.command.Command;
import chromatix.command.CommandSender;
import chromatix.command.data.CommandDataVersions;
import chromatix.command.data.CommandOverload;
import chromatix.command.data.NukkitCommandData;
...
import chromatix.entity.components.NameableComponent;
import chromatix.entity.data.human.Skin;
import chromatix.entity.data.warden.WardenWarningData;
import chromatix.entity.item.EntityFishingHook;
import chromatix.entity.item.EntityItem;
import chromatix.entity.item.EntityXpOrb;
import chromatix.entity.mob.EntityBoss;
...
import chromatix.event.block.WaterFrostEvent;
...
import chromatix.event.entity.EntityPortalEnterEvent;
import chromatix.event.entity.EntityPortalEnterEvent.PortalType;
import chromatix.event.entity.ProjectileLaunchEvent;
import chromatix.event.inventory.InventoryPickupArrowEvent;
import chromatix.event.inventory.InventoryPickupTridentEvent;
import chromatix.event.player.*;
import chromatix.event.player.PlayerInteractEvent.Action;
import chromatix.event.player.PlayerTeleportEvent.TeleportCause;
import chromatix.event.server.PacketSendEvent;
import chromatix.form.window.Form;
import chromatix.inventory.CraftTypeInventory;
import chromatix.inventory.CrafterInventory;
import chromatix.inventory.CraftingGridInventory;
import chromatix.inventory.CreativeOutputInventory;
import chromatix.inventory.EntityHandItem;
import chromatix.inventory.HumanInventory;
import chromatix.inventory.Inventory;
import chromatix.inventory.PlayerCursorInventory;
import chromatix.inventory.fake.FakeInventory;
import chromatix.inventory.fake.FakeInventoryType;
import chromatix.item.Item;
import chromatix.item.ItemArmor;
import chromatix.item.ItemArrow;
import chromatix.item.ItemBundle;
import chromatix.item.ItemID;
import chromatix.item.ItemTool;
import chromatix.item.enchantment.Enchantment;
import chromatix.lang.CommandOutputContainer;
import chromatix.lang.LangCode;
import chromatix.lang.TextContainer;
import chromatix.lang.TranslationContainer;
import chromatix.level.ChunkLoader;
import chromatix.level.GameRule;
import chromatix.level.Level;
import chromatix.level.Location;
import chromatix.level.MusicRepeatMode;
import chromatix.level.PlayerChunkManager;
import chromatix.level.Position;
import chromatix.level.Sound;
import chromatix.level.format.IChunk;
import chromatix.level.particle.BreakBlockParticle;
...
import chromatix.metadata.MetadataValue;
...
import chromatix.network.primitiveshape.PrimitiveShapes;
import chromatix.network.process.PacketHandler;
import chromatix.network.process.auth.ClientChainData;
import chromatix.permission.PermissibleBase;
import chromatix.permission.Permission;
import chromatix.permission.PermissionAttachment;
import chromatix.permission.PermissionAttachmentInfo;
import chromatix.plugin.InternalPlugin;
import chromatix.plugin.Plugin;
import chromatix.network.positiontracking.PositionTrackingService;
import chromatix.recipe.unlock.PlayerRecipeBook;
import chromatix.registry.Registries;
import chromatix.scheduler.AsyncTask;
import chromatix.scheduler.ServerScheduler;
import chromatix.scheduler.Task;
import chromatix.scheduler.TaskHandler;
...
import chromatix.utils.BlockIterator;
import chromatix.utils.BossBarColor;
import chromatix.utils.DummyBossBar;
import chromatix.utils.Identifier;
...
public class Player extends EntityHuman implements CommandSender, ChunkLoader, IPlayer, IScoreboardViewer {
     ...
     */
    protected int lastPlayedLevelUpSoundTime = 0;
    /**
     ...
     *
     * @return The network session of player
     */
    @NotNull
    ...
    /**
     * Converts the server-side game mode to a network packet-compatible game mode ID
     * This method addresses the discrepancy where NK Observer Mode has an ID of 3, while the original ID is 6
     *
    ...
    protected void checkGroundState(double movX, double movY, double movZ, double dx, double dy, double dz) {
        if (!this.onGround || movX != 0 || movY != 0 || movZ != 0) {
            ...
            double shrinkFactor = 0.01d;

            AxisAlignedBB realBB = this.boundingBox.shrink(shrinkFactor, shrinkFactor, shrinkFactor).clone();
    ...
    protected void checkBlockCollision() {
        ...
        setDataFlag(ActorFlags.OVER_SCAFFOLDING, scaffoldingUnder.length > 0);
        setDataFlag(ActorFlags.IN_ASCENDABLE_BLOCK, scaffolding);
        setDataFlag(ActorFlags.OVER_DESCENDABLE_BLOCK, scaffoldingUnder.length > 0);
                ...
                if (frostWalker != null && frostWalker.getLevel() > 0 && !this.isSpectator() && this.y >= 1 && this.y <= 255) {
                    ...
                    for (int coordX = this.getFloorX() - radius; coordX < this.getFloorX() + radius + 1; coordX++) {
                        for (int coordZ = this.getFloorZ() - radius; coordZ < this.getFloorZ() + radius + 1; coordZ++) {
                            ...
                            int layer = 0;
                            if ((block.isWaterLogged() && !block.canBeReplaced()) || !block.up().isAir() || (!block.isWaterLogged() && !block.getId().equals(BlockID.WATER)))
                                continue;
    ...
    public void processLogin() {
        ...
        this.timeSinceRest = this.nbt.getInt("TimeSinceRest");

        if (!this.nbt.contains("HasSeenCredits")) {
    ...
    protected void respawn() {
        ...
        this.setDataProperty(ActorDataTypes.AIR_SUPPLY, (short) 400, false);
        this.fireTicks = 0;
        this.collisionBlocks = null;
        this.noDamageTicks = 60;
    ...
    protected void sendPlayStatus(PlayStatus status, boolean immediate) {
        final PlayStatusPacket pk = new PlayStatusPacket();
        pk.setStatus(status);
    ...
    protected void onBlock(Entity entity, EntityDamageEvent e, boolean animate) {
        ...
        if (animate) {
            this.setDataFlag(ActorFlags.BLOCKED_USING_DAMAGED_SHIELD, true);
            getLevel().getScheduler().scheduleTask(InternalPlugin.INSTANCE, () -> {
                if (this.isOnline()) {
                    this.setDataFlag(ActorFlags.BLOCKED_USING_DAMAGED_SHIELD, false);
                }
            });
        }
    }
    ...
    /**
     * Retrieves message when players leave
     *
    ...
    /**
     * Used to set player abilities, corresponding to the game's player adventure settings.
     *
     * @param adventureSettings Player adventure settings
     */
    ...
    /**
     * Set allow modifying the world (after the unknown reason setting is completed, the player is not allowed to dig the blocks, but can place them)
     *
    ...
    /**
     * Retrieve the raw socket address
     *
    ...
    public boolean isUsingItem(String itemId) {
        return getLastUseTick(itemId) != -1 && this.getDataFlag(ActorFlags.USING_ITEM);
    }
    ...
    public void setLastUseTick(@NotNull String itemId, int tick) {
        ...
        this.setDataFlag(ActorFlags.USING_ITEM, true);
    }
    ...
    public void setButtonText(String text) {
        ...
        this.setDataProperty(ActorDataTypes.INTERACT_TEXT, this.buttonText);
    }
    ...
    public boolean isIgnoredByEntities() {
        return this.isCreative() || this.isSpectator() || !this.isOnline() || adventureSettings.get(AbilitiesIndex.INVULNERABLE);
    }

    @Override
    public void setInvulnerable(boolean value) {
        invulnerable = value;
        this.getAdventureSettings().set(AbilitiesIndex.INVULNERABLE, value).update();
    }
        ...
        if (this.spawned) {
            ...
            if (!this.isSpectator() && this.speed != null) {
                if (this.onGround) {
                    ...
                    // Wiki: Players landing at a sufficiently low angle and speed will not take fall damage. The critical damage angle for landing is 50°, with damage equivalent to falling directly from the highest point of the glide to the landing spot.
            ...
            if (!this.server.getSettings().miscSettings().overrideServerAuthBlockBreaking()) {
    ...
    public void sendMessage(String message) {
        ...
        pk.setMessageType(TextPacketType.RAW);
        pk.setSendersXUID("");
    ...
    public void sendCommandOutput(CommandOutputContainer container) {
        if (this.level.getGameRules().getBoolean(GameRule.SEND_COMMAND_FEEDBACK)) {
            ...
            pk.getOutputMessages().addAll(container.getMessages());
            pk.setOriginData(
                new CommandOriginData(
    ...
    public void sendRawTextMessage(RawText text) {
        final MessageOnly messageOnly = new MessageOnly();
        messageOnly.setMessage(text.toRawText());
    ...
    public void sendTranslation(String message, String[] parameters) {
        ...
        final TextPacketType type;
        final TextPacketBody body;
        if (forceTranslate) {
            type = TextPacketType.RAW;
            body = new MessageOnly();
            body.setMessage(this.server.getLanguage().tr(message, parameters));
        } else {
            type = TextPacketType.TRANSLATE;
            body = new MessageAndParams();
            body.setMessage(this.server.getLanguage().tr(message, parameters, "nukkit.", true));
            for (final String parameter : parameters) {
                ((MessageAndParams) body).getParameterList().add(
                    this.server.getLanguage().tr(parameter, parameters, "nukkit.", true)
                );
            }
        }
    ...
    public String getName() {
        return this.info.getIdentityClaims().extraData.displayName;
    }


    public LangCode getLanguageCode() {
        LangCode code = LangCode.from(this.getClientChainData().getLanguageCode());
    ...
    public void kill() {
        ...
        if (adventureSettings.get(AbilitiesIndex.INVULNERABLE))
            return;
     ...
     *
     * @param add              experience to be added
     * @param playLevelUpSound should level up sound played
     */
    ...
    public boolean attack(EntityDamageEvent source) {
        ...
        if (this.isSpectator() || this.isCreative() || this.getAdventureSettings().get(AbilitiesIndex.INVULNERABLE)) {
            return false;
    ...
    public boolean teleport(Location location, TeleportCause cause, boolean sendToSelf) {
        ...
        if (setPositionAndRotation(to, to.getYaw(), to.getPitch(), to.getHeadYaw())) {
            ...
            if (switchLevel) {
                ...
                //set nextChunkOrderRun is zero means that the next tick immediately executes the playerChunkManager#tick
                this.nextChunkOrderRun = 0;
    ...
    public void showDialogWindow(FormWindowDialog dialog, boolean book) {
        ...
        dialog.getBindEntity().setDataProperty(ActorDataTypes.HAS_NPC, true);
        dialog.getBindEntity().setDataProperty(ActorDataTypes.NPC_DATA, dialog.getSkinData());
        dialog.getBindEntity().setDataProperty(ActorDataTypes.ACTIONS, actionJson);
        dialog.getBindEntity().setDataProperty(ActorDataTypes.INTERACT_TEXT, dialog.getContent());
     ...
     *
     * @param window to show on the settings page
    ...
    /**
     * @since 1.21.90 (818)
     * The client closes inventories when the SLEEP player tag is set.
     * Even the player inventory, which cannot be closed with the ContainerClosePacket.
    ...
    /**
     * Whether the player is breaking block
     *
     * @return boolean
     */
    ...
    /**
     * Show a window of an XBOX account's profile
     *
     * @param xuid the XUID
     */
    ...
    public void sendAnnouncement(String source, String message) {
        ...
        final TextPacket pk = new TextPacket();
        pk.setMessageType(TextPacketType.ANNOUNCEMENT);
        pk.setSendersXUID("");
        pk.setBody(body);
     ...
     *
     * @return a copy of currently active {@link ClientInputLockComponent}s
     */
    public EnumSet<ClientInputLockComponent> getClientInputLocks() {
     ...
     * </p>
     *
     * @param flag the {@link ClientInputLockComponent} to add
     */
    public void addClientInputLock(ClientInputLockComponent flag) {
     ...
     * </p>
     *
     * @param flag the {@link ClientInputLockComponent} to remove
     */
    public void removeClientInputLock(ClientInputLockComponent flag) {
    ...
    public String getPartyId() {
        return this.clientChainData.getPartyId();
    }
    ...
    protected void syncInventory(boolean sendArmorDamage) {
        ...
        if (player != null) {
            ...
            //Send bundle content
            this.playerHandle.getWindows().keySet().stream().filter(inv -> !(inv instanceof CreativeOutputInventory)).forEach(inventory -> {
                for (int index : inventory.getContents().keySet()) {
                    Item item = inventory.getUnclonedItem(index);
                    if (item instanceof ItemBundle bundle) {
                        if (bundle.hasNbt()) {
