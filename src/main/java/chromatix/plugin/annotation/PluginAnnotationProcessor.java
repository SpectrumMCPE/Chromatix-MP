...
@SupportedAnnotationTypes({
        "chromatix.plugin.annotation.PluginMeta",
        "chromatix.plugin.annotation.EventListener",
        "chromatix.plugin.annotation.ScheduleTask",
        "chromatix.plugin.annotation.CommandDefinition"
})
public class PluginAnnotationProcessor extends AbstractProcessor {
    ...
    private void collectListener(Element e, List<String> out) {
        ...
        if (!isAssignableTo(type, "chromatix.event.Listener")) {
            error(e, "@EventListener class must implement chromatix.event.Listener.");
            return;
        }
    ...
    private void collectTask(Element e, List<ClassTask> classOut, List<MethodTask> methodOut) {
        ...
        if (e.getKind() == ElementKind.CLASS) {
            ...
            if (!isAssignableTo(type, "java.lang.Runnable")) {
                error(e, "@ScheduleTask class must implement Runnable (e.g. extend chromatix.scheduler.Task).");
                return;
            }
    ...
    private void collectCommand(Element e, List<CommandEntry> out) {
        ...
        if (!isAssignableTo(type, "chromatix.command.Command")) {
            error(e, "@CommandDefinition class must extend chromatix.command.Command.");
            return;
        }
    ...
    private boolean validateMain(TypeElement main) {
        ...
        if (!isAssignableTo(main, "chromatix.plugin.PluginBase")) {
            error(main, "@PluginMeta class must extend chromatix.plugin.PluginBase.");
            return false;
        }
        return true;
    }
    ...
    private void writeBootstrap(TypeElement main, String pkg, String bootstrapFqn,
        ...
        sb.append("    public static void init(chromatix.plugin.Plugin plugin) {\n");
        if (!listeners.isEmpty()) {
            sb.append("        chromatix.plugin.PluginManager __pm = plugin.getServer().getPluginManager();\n");
        }
        if (!classTasks.isEmpty() || !methodTasks.isEmpty()) {
            sb.append("        chromatix.scheduler.ServerScheduler __sch = plugin.getServer().getScheduler();\n");
        }
        ...
        if (!commands.isEmpty()) {
            sb.append("        chromatix.command.CommandMap __cmap = plugin.getServer().getCommandMap();\n");
