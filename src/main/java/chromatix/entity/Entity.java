...
import chromatix.block.BlockBubbleColumn;
import chromatix.block.BlockEndPortal;
import chromatix.block.BlockFence;
import chromatix.block.BlockFire;
import chromatix.block.BlockFlowingLava;
import chromatix.block.BlockFlowingWater;
import chromatix.block.BlockID;
import chromatix.block.BlockTurtleEgg;
import chromatix.blockentity.BlockEntityPistonArm;
import chromatix.entity.ai.memory.CoreMemoryTypes;
import chromatix.entity.components.*;
import chromatix.entity.custom.CustomEntity;
import chromatix.entity.custom.CustomEntityComponents;
import chromatix.entity.custom.CustomEntityDefinition;
...
import chromatix.entity.data.property.FloatEntityProperty;
...
import chromatix.nbt.tag.NumberTag;
import chromatix.nbt.tag.StringTag;
import chromatix.plugin.Plugin;
import chromatix.registry.EntityRegistry;
import chromatix.registry.Registries;
import chromatix.scheduler.Task;
import chromatix.tags.ItemTags;
import it.unimi.dsi.fastutil.longs.LongOpenHashSet;
import chromatix.utils.ChunkException;
...
public abstract class Entity extends Location implements Metadatable, EntityID {
    ...
    protected final ActorDataMap actorDataMap = new ActorDataMap();
    ...
    protected final void init(IChunk chunk, CompoundTag nbt) {
        ...
        if (this.nbt.contains("IntProperties")) {
            ...
            for (Map.Entry<String, chromatix.nbt.tag.Tag> entry : intProps.getTags().entrySet()) {
        ...
        if (this.nbt.contains("FloatProperties")) {
            ...
            for (Map.Entry<String, chromatix.nbt.tag.Tag> entry : floatProps.getTags().entrySet()) {
                String key = entry.getKey();
                if (entry.getValue() instanceof NumberTag<?> numTag) {
    ...
    public boolean dismountEntity(Entity entity, boolean sendLinks, boolean riderInitiated) {
        ...
        if (ev.isCancelled()) {
            if (seatIndex == 0) {
                broadcastLinkPacket(entity, ActorLinkType.RIDING, riderInitiated);
            } else if (seatIndex != -1) {
                broadcastLinkPacket(entity, ActorLinkType.PASSENGER, riderInitiated);
            }
            return false;
        }
        ...
        if (sendLinks) {
            broadcastLinkPacket(entity, ActorLinkType.NONE, riderInitiated);
        }
     ...
     * Used for {@link chromatix.entity.passive.EntityBat}s only.
     *
    ...
    public ActorDataMap getActorDataMap() {
