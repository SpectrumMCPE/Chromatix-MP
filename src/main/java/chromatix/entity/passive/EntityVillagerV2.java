...
import chromatix.blockentity.BlockEntityBed;
...
import chromatix.entity.ai.executor.NearbyFlatRandomRoamExecutor;
import chromatix.entity.ai.executor.PlaySoundExecutor;
import chromatix.entity.ai.executor.villager.DoorExecutor;
import chromatix.entity.ai.executor.villager.GossipExecutor;
import chromatix.entity.ai.executor.villager.SleepExecutor;
import chromatix.entity.ai.executor.villager.VillagerBreedingExecutor;
import chromatix.entity.ai.executor.villager.WillingnessExecutor;
import chromatix.entity.ai.executor.villager.WorkExecutor;
import chromatix.entity.ai.memory.CoreMemoryTypes;
import chromatix.entity.ai.route.finder.impl.SimpleFlatAStarRouteFinder;
import chromatix.entity.ai.route.posevaluator.DoorCapableWalkingPosEvaluator;
...
import chromatix.inventory.TradeInventory;
import chromatix.item.Item;
import chromatix.level.Level;
import chromatix.level.ParticleEffect;
import chromatix.level.Sound;
import chromatix.level.format.IChunk;
import chromatix.level.village.VillageDwellers;
...
public class EntityVillagerV2 extends EntityIntelligent implements InventoryHolder, IEntityNPC {
                        ...
                        entity -> {
                            if (getLevel().getTick() % 30 == 0 && !isBaby()) {
                                ...
                                if (siteBlock != null && !siteBlock.getLevelBlock().getId().equals(siteBlock.getId())) {
                                    getLevel().getVillageManager().release(siteBlock.asBlockVector3());
                                ...
                                if (getMemoryStorage().isEmpty(CoreMemoryTypes.SITE_BLOCK)) {
                                    if(getBed() != null) {
                                        ...
                                        if (!acquired && getTradeExp() == 0 && getVillageUuid() != null) {
    ...
    public @Nullable UUID getVillageUuid() {
        if (!this.nbt.containsString("villageUuid")) {
            return null;
        }
        try {
            return UUID.fromString(this.nbt.getString("villageUuid"));
        } catch (IllegalArgumentException ignored) {
            return null;
        }
    }

    public void setVillageUuid(UUID villageUuid) {
    ...
    public void close() {
        if (getLevel() != null && getMemoryStorage() != null) {
            BlockBed bed = getMemoryStorage().get(CoreMemoryTypes.OCCUPIED_BED);
            if (bed != null) getLevel().getVillageManager().release(bed.asBlockVector3());
            Block site = getMemoryStorage().get(CoreMemoryTypes.SITE_BLOCK);
            if (site != null) getLevel().getVillageManager().release(site.asBlockVector3());
        }
