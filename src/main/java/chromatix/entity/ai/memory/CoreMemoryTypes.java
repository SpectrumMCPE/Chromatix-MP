package chromatix.entity.ai.memory;
...
import chromatix.entity.ai.memory.codec.BooleanMemoryCodec;
import chromatix.entity.ai.memory.codec.NumberMemoryCodec;
import chromatix.entity.ai.memory.codec.StringMemoryCodec;
...
public interface CoreMemoryTypes {
    ...
    MemoryType<Entity> NEAREST_SHARED_ENTITY = new MemoryType<>("minecraft:nearest_shared_entity");
        ...
        MemoryType<Class<?>> LOOKING_BLOCK = new MemoryType<>("minecraft:looking_block");
    ...
    /**
     * Represents the state of anger and is bound to {@link ActorFlags#ANGRY} <p>
    ...
    MemoryType<Boolean> IS_ANGRY = new MemoryType<>("minecraft:is_angry", false)
            .withCodec(new BooleanMemoryCodec("Angry")
                    .onInit((data, entity) -> entity.setDataFlag(ActorFlags.ANGRY, data))
            );
    /**
     * The state representing whether an entity is sitting is bound to {@link ActorFlags#SITTING}. <p>
     * Currently only used in Wolf.
     */
    MemoryType<Boolean> IS_SITTING = new MemoryType<>("minecraft:is_sitting", false)
            .withCodec(new BooleanMemoryCodec("Sitting")
                    .onInit((data, entity) -> {
                        entity.setDataFlag(ActorFlags.SITTING, data);
                    })
            );
    /**
     * Represents the entity owner and is bound to {@link ActorFlags#TAMED} and {@link ActorDataTypes#OWNER}. <p>
     * Currently only used in Wolf.
     */
    MemoryType<String> OWNER_NAME = new MemoryType<String>("minecraft:owner_name")
            .withCodec(new StringMemoryCodec("OwnerName")
                    .onInit((data, entity) -> {
                        if (data == null) {
                            entity.setDataProperty(ActorDataTypes.OWNER, 0L);
                            entity.setDataFlag(ActorFlags.TAMED, false);
                        } else {
    ...
    /**
     * The color representing the entity is bound to {@link ActorDataTypes#COLOR_INDEX}. <p>
    ...
    /**
     * The second color representing the entity is bound to {@link ActorDataTypes#COLOR_2_INDEX}. <p>
