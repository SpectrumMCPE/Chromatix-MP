...
import chromatix.entity.mob.EntityElderGuardian;
...
import chromatix.level.generator.object.structures.utils.NukkitCollections;
...
public class OceanMonumentPieces {
    ...
    private static final BlockState PRISMARINE = BlockPrismarine.PROPERTIES.getDefaultState();
    private static final BlockState PRISMARINE_BRICKS = BlockPrismarineBricks.PROPERTIES.getDefaultState();
    private static final BlockState DARK_PRISMARINE = BlockDarkPrismarine.PROPERTIES.getDefaultState();
    private static final BlockState SEA_LANTERN = BlockSeaLantern.PROPERTIES.getDefaultState();
    private static final BlockState WATER = BlockWater.PROPERTIES.getDefaultState();
    private static final BlockState FLOWING_WATER = BlockFlowingWater.PROPERTIES.getDefaultState();
    private static final BlockState WET_SPONGE = BlockWetSponge.PROPERTIES.getDefaultState();
    private static final BlockState GOLD_BLOCK = BlockGoldBlock.PROPERTIES.getDefaultState();
    private static final BlockState ICE = BlockIce.PROPERTIES.getDefaultState();
    private static final BlockState PACKED_ICE = BlockPackedIce.PROPERTIES.getDefaultState();

    interface MonumentRoomFitter {

        boolean fits(RoomDefinition roomDefinition);

        OceanMonumentPiece create(BlockFace orientation, RoomDefinition roomDefinition, RandomSourceProvider random);
    }
    ...
    public abstract static class OceanMonumentPiece extends StructurePiece {
        ...
        protected void generateWaterBox(BlockManager level, BoundingBox boundingBox, int x0, int y0, int z0, int x1, int y1, int z1) {
            for (int y = y0; y <= y1; ++y) {
                for (int x = x0; x <= x1; ++x) {
                    for (int z = z0; z <= z1; ++z) {
        ...
        protected void generateDefaultFloor(BlockManager level, BoundingBox boundingBox, int x, int z, boolean hasOpening) {
            if (hasOpening) {
        ...
        protected void generateBoxOnFillOnly(BlockManager level, BoundingBox boundingBox, int x0, int y0, int z0, int x1, int y1, int z1, BlockState block) {
            for (int y = y0; y <= y1; ++y) {
                for (int x = x0; x <= x1; ++x) {
                    for (int z = z0; z <= z1; ++z) {
                        BlockState current = this.getBlock(level, x, y, z, boundingBox);
                        if (current.getIdentifier() == Block.WATER || current.getIdentifier() == Block.FLOWING_WATER) { // == FILL_BLOCK
        ...
        protected boolean spawnElder(BlockManager level, BoundingBox boundingBox, int x, int y, int z) {
    ...
    public static class MonumentBuilding extends OceanMonumentPiece {
        ...
        public MonumentBuilding(RandomSourceProvider random, int x, int z, BlockFace orientation) {
            super(0);
        ...
        private List<RoomDefinition> generateRoomGraph(RandomSourceProvider random) {
            ...
            (this.coreRoom = roomDefinitions[getRoomIndex(random.nextBoundedInt(3), 0, 2)]).claimed = true;
        ...
        private void generateWing(boolean reverse, int x, BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateEntranceArchs(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateEntranceWall(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateRoofPiece(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateLowerWall(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateMiddleWall(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
        ...
        private void generateUpperWall(BlockManager level, RandomSourceProvider random, BoundingBox boundingBox) {
    ...
    public static class OceanMonumentSimpleRoom extends OceanMonumentPiece {
        ...
        public OceanMonumentSimpleRoom(BlockFace orientation, RoomDefinition roomDefinition, RandomSourceProvider random) {
            super(1, orientation, roomDefinition, 1, 1, 1);
    ...
    static class FitSimpleRoom implements MonumentRoomFitter {
        ...
        @Override
        public OceanMonumentPiece create(BlockFace orientation, RoomDefinition roomDefinition, RandomSourceProvider random) {
