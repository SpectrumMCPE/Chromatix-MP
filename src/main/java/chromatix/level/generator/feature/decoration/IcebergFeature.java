...
import static chromatix.block.BlockID.BLUE_ICE;
import static chromatix.block.BlockID.FLOWING_WATER;
...
public class IcebergFeature extends GenerateFeature {
    ...
    private void generateBlueIce(BlockManager manager, int originX, int originZ) {
        ...
        setBlock(manager, x, y, z, chromatix.block.BlockBlueIce.PROPERTIES.getDefaultState());
        ...
        for (int i = 0; i < 200; i++) {
            ...
            if (placeState.equals(AIR) || isWaterState(placeState) || placeState.equals(PACKED_ICE) || placeState.equals(ICE)) {
                if (hasAdjacentBlueIce(manager, px, py, pz)) {
                    setBlock(manager, px, py, pz, chromatix.block.BlockBlueIce.PROPERTIES.getDefaultState());
                }
            }
        }
    }
