...
import chromatix.api.NonComputationAtomic;
...
import chromatix.block.customblock.CustomBlockDefinition.BlockTickSettings;
...
import chromatix.config.category.GameplaySettings;
import chromatix.event.level.ChunkTickEvent;
import chromatix.level.tickingarea.TickingArea;
import chromatix.level.tickingarea.manager.TickingAreaManager;
import chromatix.entity.Entity;
import chromatix.entity.EntityAsyncPrepare;
import chromatix.entity.EntityID;
import chromatix.entity.EntityIntelligent;
import chromatix.entity.EntityQueryOptions;
...
import chromatix.entity.item.EntityPainting;
...
import chromatix.event.block.BlockBreakEvent;
import chromatix.event.block.BlockChangeEvent;
import chromatix.event.block.BlockPlaceEvent;
import chromatix.event.block.BlockUpdateEvent;
import chromatix.event.level.ChunkLoadEvent;
import chromatix.event.level.ChunkUnloadEvent;
import chromatix.event.level.LevelSaveEvent;
import chromatix.event.level.LevelUnloadEvent;
import chromatix.event.level.SpawnChangeEvent;
import chromatix.event.level.ThunderChangeEvent;
import chromatix.event.level.WeatherChangeEvent;
import chromatix.event.player.PlayerInteractEvent;
import chromatix.event.player.PlayerInteractEvent.Action;
import chromatix.event.weather.LightningStrikeEvent;
import chromatix.inventory.BlockInventoryHolder;
import chromatix.item.Item;
import chromatix.item.enchantment.Enchantment;
import chromatix.level.format.Chunk;
...
import chromatix.level.format.ReflectiveLevelProviderFactory;
import chromatix.level.format.UnsafeChunk;
import chromatix.level.format.leveldb.LevelDBProvider;
import chromatix.level.generator.BiomedGenerator;
...
import chromatix.level.particle.Particle;
import chromatix.level.util.EntityQueryUtils;
import chromatix.level.util.SimpleTickCachedBlockStore;
import chromatix.level.util.TickCachedBlockStore;
import chromatix.level.vibration.SimpleVibrationManager;
import chromatix.level.vibration.VibrationEvent;
import chromatix.level.vibration.VibrationManager;
import chromatix.level.vibration.VibrationType;
import chromatix.level.village.VillageManager;
import chromatix.math.*;
import chromatix.math.BlockFace.Plane;
import chromatix.metadata.BlockMetadataStore;
import chromatix.metadata.MetadataValue;
import chromatix.metadata.Metadatable;
...
import chromatix.utils.collection.nb.Int2ObjectNonBlockingMap;
import chromatix.utils.collection.nb.Long2ObjectNonBlockingMap;
...
public class Level implements Metadatable {
    ...
    public void scheduleUpdate(Block block, Vector3 pos, int delay, int priority, boolean checkArea, boolean checkBlockWhenUpdate) {
        ...
        if (block.getId().equals(BlockID.AIR) || (checkArea && !this.isChunkLoaded(chunkX, chunkZ))) {
            return;
        }
    ...
    public boolean isConcurrentSchedule(Vector3 pos, Block block, long targetTick, int delay) {
        IChunk chunk = this.getChunk(NukkitMath.floorDouble(pos.x) >> 4, NukkitMath.floorDouble(pos.z) >> 4, false);
        return chunk != null && chunk.getBlockUpdateScheduler().isConcurrentSchedule(pos, block, targetTick, delay);
    }

    public boolean isBlockTickPending(Vector3 pos, Block block) {
        IChunk chunk = this.getChunk(NukkitMath.floorDouble(pos.x) >> 4, NukkitMath.floorDouble(pos.z) >> 4, false);
        return chunk != null && chunk.getBlockUpdateScheduler().isBlockTickPending(pos, block);
    }

    public Set<BlockUpdateEntry> getPendingBlockUpdates(IChunk chunk) {
        return chunk.getBlockUpdateScheduler().getPendingBlockUpdates();
    }

    public Set<BlockUpdateEntry> getPendingBlockUpdates(AxisAlignedBB boundingBox) {
        Set<BlockUpdateEntry> pending = new HashSet<>();
        int minChunkX = NukkitMath.floorDouble(boundingBox.getMinX()) >> 4;
        int maxChunkX = NukkitMath.floorDouble(boundingBox.getMaxX()) >> 4;
        int minChunkZ = NukkitMath.floorDouble(boundingBox.getMinZ()) >> 4;
        int maxChunkZ = NukkitMath.floorDouble(boundingBox.getMaxZ()) >> 4;
        for (int x = minChunkX; x <= maxChunkX; x++) {
            for (int z = minChunkZ; z <= maxChunkZ; z++) {
                IChunk chunk = this.getChunk(x, z, false);
                if (chunk != null) {
                    pending.addAll(chunk.getBlockUpdateScheduler().getPendingBlockUpdates(boundingBox));
                }
            }
        }
        return pending;
