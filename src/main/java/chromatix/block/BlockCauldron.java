...
import chromatix.blockentity.BlockEntityCauldron;
import chromatix.entity.Entity;
import chromatix.entity.effect.EffectType;
import chromatix.event.entity.EntityCombustByBlockEvent;
...
import chromatix.event.player.PlayerBucketFillEvent;
import chromatix.item.*;
import chromatix.level.Sound;
import chromatix.level.particle.SmokeParticle;
...
import static chromatix.block.property.CommonBlockProperties.CAULDRON_LIQUID;
import static chromatix.block.property.CommonBlockProperties.FILL_LEVEL;
        ...
        if (item instanceof ItemBucket bucket) {
            ...
            if (bucket.isEmpty()) {
                ...
                if (!ev.isCancelled()) {
                    ...
                    this.setFillLevel(FILL_LEVEL.getMin(), player);
                ...
                if (!ev.isCancelled()) {
                    ...
                    if (cauldron.hasPotion()) {
                        ...
                        if (!isEmpty() && getCauldronLiquid() != CauldronLiquid.POWDER_SNOW) {
                            ...
                            cauldron.clearCustomColor();
                            this.level.setBlock(this, this, true);
                            this.getLevel().addSound(this.add(0.5, 1, 0.5), Sound.BUCKET_EMPTY_POWDER_SNOW);
                        }
    ...
    public boolean onLavaActivate(@NotNull Item item, Player player, BlockFace blockFace, float fx, float fy, float fz) {
        ...
        if (item instanceof ItemBucket bucket) {
            ...
            if (bucket.isEmpty()) {
                ...
                if (isFull() && !cauldron.isCustomColor() && !cauldron.hasPotion() && bucket.isLava()) {
                    return true;
                }
                ...
                if (!ev.isCancelled()) {
                    ...
                    if (cauldron.hasPotion() || bucket.isWater() || bucket.isPowderSnow()) {
                        clearWithFizz(cauldron, player);
                    } else if (bucket.isLava()) {
                        this.setFillLevel(FILL_LEVEL.getMax(), player);
                        cauldron.clearCustomColor();
