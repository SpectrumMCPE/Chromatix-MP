...
import chromatix.education.block.*;
import chromatix.education.block.elements.*;
import chromatix.education.block.glass.*;
import chromatix.item.Item;
import chromatix.item.ItemID;
import chromatix.network.protocol.types.inventory.creative.CreativeCustomGroups;
import chromatix.registry.CreativeGroupsRegistry;
import chromatix.registry.CreativeItemRegistry;
import chromatix.registry.Registries;
import chromatix.utils.MapParsingUtils;
...
public class Education implements BlockID, ItemID {
    ...
    private static void addCreativeGroup(String name, String icon) {
        ...
        payload.setCreativeNetId(new CreativeItemNetId(item.getNetId()));
        payload.setItemInstance(item.toNetwork());
        payload.setGroupIndex(0);
            ...
            try (InputStreamReader reader = new InputStreamReader(input)) {
                ...
                for (String group : tmpGroups) {
                    ...
                    CreativeCustomGroups.getDefinedGroups().stream().filter(d -> d.getName().equalsIgnoreCase(group)).findFirst().flatMap(def -> CreativeItemRegistry.ITEM_DATA.stream().filter(d -> Item.fromNetwork(d.getItemInstance()).getId().equalsIgnoreCase(def.getIconId())).findFirst()).ifPresent(entry -> {
                        ...
                        final CreativeItemEntryPayload payload = new CreativeItemEntryPayload();
                        payload.setCreativeNetId(entry.getCreativeNetId());
                        payload.setItemInstance(entry.getItemInstance());
                        payload.setGroupIndex(groups.get(group));
                        CreativeItemRegistry.ITEM_DATA.add(payload);
                    });
                }
