...
public class TickCachedBlockIterator implements Iterator<Block> {
    ...
    public TickCachedBlockIterator(Level level, Vector3 start, Vector3 direction) {
        this(level, start, direction, 0);
    }

    public TickCachedBlockIterator(Level level, Vector3 start, Vector3 direction, double yOffset) {
    ...
    public TickCachedBlockIterator(Level level, Vector3 start, Vector3 direction, double yOffset, int maxDistance) {
        ...
        Block lastBlock = startBlock.getTickCachedSide(this.mainFace.getOpposite());

        if (this.secondError < 0) {
            this.secondError += gridSize;
            lastBlock = lastBlock.getTickCachedSide(this.secondFace.getOpposite());
        }

        if (this.thirdError < 0) {
            this.thirdError += gridSize;
            lastBlock = lastBlock.getTickCachedSide(this.thirdFace.getOpposite());
        }
    ...
    private void scan() {
        ...
        if (this.secondError > 0 && this.thirdError > 0) {
            this.blockQueue[2] = this.blockQueue[0].getTickCachedSide(this.mainFace);

            if ((this.secondStep * this.thirdError) < (this.thirdStep * this.secondError)) {
                this.blockQueue[1] = this.blockQueue[2].getTickCachedSide(this.secondFace);
                this.blockQueue[0] = this.blockQueue[1].getTickCachedSide(this.thirdFace);
            } else {
                this.blockQueue[1] = this.blockQueue[2].getTickCachedSide(this.thirdFace);
                this.blockQueue[0] = this.blockQueue[1].getTickCachedSide(this.secondFace);
            }
            ...
            this.blockQueue[1] = this.blockQueue[0].getTickCachedSide(this.mainFace);
            this.blockQueue[0] = this.blockQueue[1].getTickCachedSide(this.secondFace);
            this.secondError -= gridSize;
            this.currentBlock = 1;
            ...
            this.blockQueue[0] = this.blockQueue[0].getTickCachedSide(this.mainFace);
            this.currentBlock = 0;
        }
    }
}
