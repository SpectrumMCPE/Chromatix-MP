package chromatix.utils.collection.nb;
 ...
 * Int2ObjectNonBlockingMap} scales substatially better than {@link
 ...
 * chromatix.utils.collection.nb.Int2ObjectNonBlockingMap} with {@link Integer} keys is
...
public class Int2ObjectNonBlockingMap<TypeV>
    ...
    @Serial
    private static final long serialVersionUID = 1484063708153648087L;
    ...
    private static final VarHandle _IHandler = MethodHandles.arrayElementVarHandle(int[].class);
    ...
    // I exclude 1 int from the 2^64 possibilities, and test for it before
    // entering the main array.  The NO_KEY value must be zero, the initial
    // value set by Java before it hands me the array.
    private static final int NO_KEY = 0;

    // Count of reprobes
    private transient ConcurrentAutoIntTable _reprobes = new ConcurrentAutoIntTable();
    ...
    /**
     * Create a new Int2ObjectNonBlockingMap with default minimum size (currently set
     * to 8 K/V pairs or roughly 84 bytes on a standard 32-bit JVM).
     */
    public Int2ObjectNonBlockingMap() {
        this(MIN_SIZE, true);
    }

    /**
     * Create a new Int2ObjectNonBlockingMap with initial room for the given
     ...
     */
    public Int2ObjectNonBlockingMap(final int initial_sz) {
        this(initial_sz, true);
    }

    /**
     * Create a new Int2ObjectNonBlockingMap, setting the space-for-speed
     ...
     * speed improvement.
     */
    public Int2ObjectNonBlockingMap(final boolean opt_for_space) {
        this(1, opt_for_space);
    }

    /**
     * Create a new Int2ObjectNonBlockingMap, setting both the initial size and
     ...
     */
    public Int2ObjectNonBlockingMap(final int initial_sz, final boolean opt_for_space) {
    ...
    @SuppressWarnings("unchecked")
    public final TypeV get(int key) {
        if (key == NO_KEY) {
    ...
    private static final class CHM implements Serializable {
        ...
        private boolean CAS_key(int idx, int old, int key) {
            // return UNSAFE.compareAndSwapLong(_keys, rawIndex(_keys, idx), old, key);
            return _IHandler.compareAndSet(_keys, idx, old, key);
        }
        ...
        CHM(final Int2ObjectNonBlockingMap<?> nbhml, ConcurrentAutoIntTable size, final int logsize) {
            ...
            _keys = new int[1 << logsize];
            _vals = new Object[1 << logsize];
        }

        // Non-atomic clear
        void clear() {
            _size = new ConcurrentAutoIntTable();
            _slots = new ConcurrentAutoIntTable();
            Arrays.fill(_keys, 0);
        ...
        // --- get_impl ----------------------------------------------------------
        // Never returns a Prime nor a Tombstone.
        private Object get_impl(final int key) {
            ...
            int reprobe_cnt = 0;
            while (true) {
                final int K = _keys[idx]; // Get key   before volatile read, could be NO_KEY
        ...
        private Object putIfMatch(final int key, final Object putval, final Object expVal) {
            final int hash = hash(key);
            assert putval != null;
    ...
    private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
        ...
        public void removeKey() {
            ...
            Int2ObjectNonBlockingMap.this.putIfMatch(_prevK, TOMBSTONE, NO_MATCH_OLD);
            _prevV = null;
        }
    ...
    // new NBHMLEntry.
    private class NBHMLEntry implements IntObjectEntry<TypeV>, Entry<Integer, TypeV> {
        private final int k;
        private TypeV v;

        NBHMLEntry(final int k, final TypeV v) {
            this.k = k;
            this.v = v;
        }
    ...
    private class SnapshotE implements Iterator<IntObjectEntry<TypeV>> {
        final SnapshotV _ss;
    ...
    private class SnapshotESlow implements Iterator<Entry<Integer, TypeV>> {
        final SnapshotV _ss;
     ...
     * iteration.  The {@link chromatix.utils.collection.nb.Int2ObjectNonBlockingMap}
    ...
    @SuppressWarnings("unchecked")
    @Override
    public Int2ObjectNonBlockingMap<TypeV> clone() {
        try {
            // Must clone, to get the class right; NBHML might have been
            // extended so it would be wrong to just make a new NBHML.
            Int2ObjectNonBlockingMap<TypeV> t = (Int2ObjectNonBlockingMap<TypeV>) super.clone();
     ...
     */
    @Override
    public TypeV computeIfAbsent(Integer key, Function<? super Integer, ? extends TypeV> mappingFunction) {
     ...
     */
    @Override
    public TypeV compute(Integer key, BiFunction<? super Integer, ? super TypeV, ? extends TypeV> remappingFunction) {
