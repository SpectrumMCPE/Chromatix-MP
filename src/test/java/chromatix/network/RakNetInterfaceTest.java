            ...
            protected void initChannel(RakChildChannel ch) throws Exception {
                ...
                ch.pipeline().addLast(new SimpleChannelInboundHandler<RakMessage>() {
                    ...
                    protected void channelRead0(ChannelHandlerContext ctx, RakMessage msg) throws Exception {
                        ...
                        //batch packet
                        int packetLength = VarInts.readUnsignedInt(content);
                        ByteBuf byteBuf = content.readSlice(packetLength);

                        //decode to game packet
                        int header = VarInts.readUnsignedInt(byteBuf);
                        assert header == NetworkConstants.CODEC.getPacketDefinition(RequestNetworkSettingsPacket.class).getId();
                        final RequestNetworkSettingsPacket packet = new RequestNetworkSettingsPacket();
                        NetworkConstants.CODEC.getPacketDefinition(RequestNetworkSettingsPacket.class).getSerializer()
                                .deserialize(byteBuf, NetworkConstants.CODEC.createHelper(), packet);
                        assert packet.getClientNetworkVersion() == NetworkConstants.CODEC.getProtocolVersion();
                        ...
                        ch.pipeline()
                                .addLast(new ChannelInboundHandlerAdapter() {
                                    ...
                                    public void channelActive(ChannelHandlerContext ctx) throws Exception {
                                        ...
                                        VarInts.writeUnsignedInt(buf, NetworkConstants.CODEC.getPacketDefinition(RequestNetworkSettingsPacket.class).getId());//packet header
                                        buf.writeInt(NetworkConstants.CODEC.getProtocolVersion());
                                        //RequestNetworkSettingsPacket
